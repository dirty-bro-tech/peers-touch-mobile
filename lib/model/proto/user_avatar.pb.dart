...
/// UserAvatarService provides operations for managing user profile avatars
class UserAvatarServiceApi {
  $pb.RpcClient _client;
  UserAvatarServiceApi(this._client);

  /// UploadAvatar uploads a new profile avatar for a user
  /// Note: Actual implementation uses multipart/form-data for the upload
  $async.Future<UploadAvatarResponse> uploadAvatar($pb.ClientContext? ctx, UploadAvatarRequest request) =>
    _client.invoke<UploadAvatarResponse>(ctx, 'UserAvatarService', 'UploadAvatar', request, UploadAvatarResponse())
  ;
  /// GetUserAvatars retrieves a user's previous avatars (most recent first)
  $async.Future<GetUserAvatarsResponse> getUserAvatars($pb.ClientContext? ctx, GetUserAvatarsRequest request) =>
    _client.invoke<GetUserAvatarsResponse>(ctx, 'UserAvatarService', 'GetUserAvatars', request, GetUserAvatarsResponse())
  ;
}
